...
export function getPaddle(
  onCheckoutEvent?: (event: CheckoutEventsData) => void
  ...
  switch ((event as any).name) {
    ...
    case 'checkout.completed': {
      ...
      trackEvent('checkout_completed_client', {
        transactionId: (event as any).data?.transaction_id,
      });

      const txnId = (event as any).data?.transaction_id;

      const productId = (
        (event as any).data?.custom_data as
...
export async function createCheckout({
  ...
  if (!paddle) {
    throw new Error(
      'Paddle failed to initialize. Check NEXT_PUBLIC_PADDLE_CLIENT_TOKEN and network access.'
    );
  }
